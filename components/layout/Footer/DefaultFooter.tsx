const FooterItemLink = ({ link, text }: any) => {
  return (
    <div>
      <a
        href={link}
        className="group relative text-sm text-white hover:text-neutral-400 transition duration-300"
      >
        {text}

        {/* underline animation */}
        <span className="absolute left-0 -bottom-1 w-0 h-[1px] bg-neutral-400 transition-all duration-300 group-hover:w-full"></span>
      </a>
    </div>
  )
}
...
export default function DefaultFooter({ sections = [], social = [] }: any) {
  return (
    <footer className="relative w-full mt-10 bg-neutral-900 backdrop-blur-md text-white overflow-hidden shadow-2xl">
      ...
      <div className="relative max-w-screen-2xl mx-auto py-10 sm:py-12 md:py-16 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-12 lg:gap-16">
          ...
          <div className="md:col-span-2">
            <h3 className="text-2xl font-bold mb-6 tracking-tight bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">
              JOIN OUR NEWSLETTER
            </h3>

            <p className="text-neutral-400 text-sm mb-10 max-w-sm leading-relaxed font-medium">
            ...
            <div className="group relative flex w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl transition-all duration-500 hover:bg-white/10 hover:shadow-neutral-500/10 focus-within:bg-white/15 focus-within:ring-2 focus-within:ring-neutral-400/50 focus-within:scale-[1.01]">
              <input
                type="email"
                placeholder="Enter your email"
                className="flex-1 bg-transparent px-5 py-4 text-sm text-white placeholder:text-white/50 outline-none w-full"
              />
              <button className="bg-neutral-800 hover:bg-neutral-700 px-8 text-sm font-bold tracking-wide transition-all duration-300 shadow-lg hover:shadow-neutral-500/20 active:scale-95 border-l border-white/10">
                Subscribe
              </button>
            </div>
          </div>
          ...
          <div className="md:col-span-4 grid grid-cols-2 lg:grid-cols-4 gap-8">
            {sections.map((section: any) => (
              <div key={section.title} className="col-span-1">
                <h4 className="text-neutral-100 uppercase tracking-[0.2em] text-[12px] font-black mb-8">
                  {section.title}
                </h4>
        ...
        {/* DIVIDER */}
        <div className="mt-10 pt-8 border-t border-white/5 flex flex-col items-center"></div>

        {/* BOTTOM BAR */}
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 text-sm">
          {/* LEFT LINKS */}
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 text-neutral-400 font-medium">
          ...
          <div className="flex items-center gap-6">
            {social.map((item: any, index: number) => (
              <a
                ...
                href={item.link}
                target="_blank"
                className="group relative flex h-10 w-10 items-center justify-center rounded-full bg-white/5 border border-white/10 hover:border-neutral-500/50 transition-all duration-500 overflow-hidden"
              >
          ...
          <p className="text-neutral-500 font-medium tracking-tight">
            © {new Date().getFullYear()} <span className="text-neutral-400">Fashion Cloth</span>.
          </p>
        </div>
      </div>
    </footer>
